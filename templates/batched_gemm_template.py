"""Pre-verified HipKittens Batched GEMM template"""
import torch
...
torch::Tensor bmm_forward(torch::Tensor A, torch::Tensor B);
'''
...
// Same kernel as GEMM, but called per batch
...
torch::Tensor bmm_forward(torch::Tensor A, torch::Tensor B) {
    ...
    int batch_size = A_cont.size(0);
    int M = A_cont.size(1);
    int K = A_cont.size(2);
    int N = B_cont.size(2);
    
    auto C = torch::empty({batch_size, M, N}, A_cont.options());
    
    // Process each batch
    for (int b = 0; b < batch_size; b++) {
        auto A_batch = A_cont.select(0, b);
        auto B_batch = B_cont.select(0, b);
        auto C_batch = C.select(0, b);
        
        // Transpose B for mma_ABt
        auto Bt = B_batch.t().contiguous();
        
        _gl_bf16 gA{(bf16*)A_batch.data_ptr<at::BFloat16>(), 1u, 1u, (unsigned)M, (unsigned)K};
        _gl_bf16 gB{(bf16*)Bt.data_ptr<at::BFloat16>(), 1u, 1u, (unsigned)N, (unsigned)K};
        _gl_bf16 gC{(bf16*)C_batch.data_ptr<at::BFloat16>(), 1u, 1u, (unsigned)M, (unsigned)N};
